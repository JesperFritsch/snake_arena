...
Drafts of the user-facing legal documents for snake_arena.
...
- `<DOMAIN>` — the public domain the service will run on (e.g. `snakearena.example`). If still undecided, leave `<DOMAIN>` in until the domain is registered.
- `<EFFECTIVE_DATE>` — the date the documents go live (ISO 8601, e.g. `2026-06-15`).

## Files

- `terms_of_service.md` — the contract between snake_arena and its users.
